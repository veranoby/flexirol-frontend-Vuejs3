...
export const useSystemConfigStore = defineStore('systemConfig', () => {
  ...
  async function fetchConfig(forceRefresh = false) {
    ...
    loading.value = true
    try {
      const result = await api.pb.collection('system_config')
  ...
  async function updateConfig(updates) {
    try {
      const current = await api.pb.collection('system_config')
        .getFirstListItem('name="default_config"')

      const updated = await api.pb.collection('system_config')
