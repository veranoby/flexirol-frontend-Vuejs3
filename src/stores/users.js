...
export const useUsersStore = defineStore('users', () => {
  ...
  const activeUsers = computed(() => 
    users.value.filter(user => user.gearbox === true)
  )

  const usersByCompany = computed(() => (companyId) => 
    users.value.filter(user => user.empresa_id === companyId)
  )
  ...
  const validateUserData = async (userData, isUpdate = false, userId = null) => {
    ...
    const emailExists = await api.collection('users').getFirstListItem(emailFilter, { requestKey: null })
      .catch(() => null)
    ...
    const cedulaExists = await api.collection('users').getFirstListItem(cedulaFilter, { requestKey: null })
      .catch(() => null)
  ...
  async function createUser(userData) {
    ...
    try {
      ...
      // Generate username using system store
      ...
      const user = {
        ...
        password: userData.password || userData.cedula, // Default to cédula as password
  ...
  async function updateUser(userId, userData) {
    ...
    try {
      ...
      const updateData = {
        ...
        empresa_id: userData.role === 'empresa' ? null : (userData.empresa_id || existingUser.empresa_id)
      }
